      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-12">
            <h2 className="text-3xl md:text-5xl font-bold text-white uppercase tracking-wider">
               {/* Ici on met "LE PROJET" en blanc et "MARS.A.I" en orange */}
            ...
            <p className="text-gray-400 mt-4 max-w-2xl text-lg font-medium">
              {t('section_mars_subtitle')} {/* Match JSON ✅ */}
            </p>
        </div>
          ...
          {events.map((card) => (
            <div key={card.id} className="relative h-96 rounded-3xl overflow-hidden group shadow-xl border border-slate-800">
              ...
              <div className="absolute inset-0 p-8 flex flex-col justify-end items-start">
                <h3 className="text-2xl font-bold text-white mb-3 leading-tight uppercase">
                  {card.title}
                </h3>
                <p className="text-gray-300 text-sm mb-6 font-medium border-t border-gray-600/50 pt-4 w-full leading-relaxed italic">
                  {card.description}
                </p>
