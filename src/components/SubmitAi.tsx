...
export default function SubmitAi({ ia, handleChange }: { 
    ia: Ia, 
  ...
  return (
    <section className="border border-[#364153] rounded-2xl p-5 my-10 font-display">
      ...
      <div className="flex flex-col md:flex-row justify-center mt-8 gap-4 px-7">
        ...
        >
          <input
            ...
            className="hidden"
          />
          <h3 className="font-bold mb-1 text-sm">{t('submit_ia.full_ai_title', '100% IA')}</h3>
          <p className="text-xs text-gray-400">{t('submit_ia.full_ai_desc', 'Contenu entièrement généré')}</p>
        </label>
        ...
        >
          <input
            ...
            className="hidden"
          />
          <h3 className="font-bold mb-1 text-sm">{t('submit_ia.hybrid_title', 'Production Hybride')}</h3>
          <p className="text-xs text-gray-400">{t('submit_ia.hybrid_desc', 'Réel + IA')}</p>
        </label>
      </div>

      {/* STACK TECHNOLOGIQUE */}
      <div className="mx-7 mt-8">
        <label className={labelClasses}>{t('submit_ia.stack_label', 'Stack Technologique')}</label>
        <textarea
          name="stack"
          value={ia.stack} 
      ...
      <div className="mx-7 mt-8">
        <label className={labelClasses}>{t('submit_ia.method_label', 'Méthodologie Créative')}</label>
        <textarea
